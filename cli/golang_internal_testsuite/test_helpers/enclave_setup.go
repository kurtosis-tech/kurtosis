...
func CreateEnclave(t *testing.T, ctx context.Context, testName string) (resultEnclaveCtx *enclaves.EnclaveContext, resultDestroyEnclaveFunc func(), resultErr error) {
	kurtosisCtx, err := kurtosis_context.NewKurtosisContextFromLocalEngine()
	assert.NoError(t, err, "An error occurred connecting to the Kurtosis engine for running test '%v'", testName)
	...
	return enclaveCtx, destroyEnclaveFunc, nil
}
