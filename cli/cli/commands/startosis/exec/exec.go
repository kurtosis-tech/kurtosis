...
import (
	...
	"github.com/kurtosis-tech/kurtosis/cli/cli/helpers/module_image_name_generator"
	...
	Flags: []*flags.FlagConfig{
		...
		},
		{
			...
			Usage: fmt.Sprintf(
				...
				module_image_name_generator.AllowedEnclaveIdCharsRegexStr, //TODO change this indication and move the const to another package
			),
			Type:    flags.FlagType_String,
...
func run(
	...
	err = module_image_name_generator.ValidateImageName(enclaveIdStr) //TODO remove this validation from here, the enclave ID validation was moved to the engine's server
	if err != nil {
...
func getAutoGeneratedEnclaveIdStr(scriptFilePath string) string {
	...
	filenameMaxLength := module_image_name_generator.ModuleImageNameMaxLength - len(epochStrSuffix)
