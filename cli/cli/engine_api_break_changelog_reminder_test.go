...
const (
	...
	expectedEngineMajorMinorVersion = "1.7"
	// !!!!!! BEFORE YOU UPDATE THIS CONSTANT TO FIX THE TEST, ADD A "BREAKING CHANGE" SECTION IN THE CHANGELOG !!!!!!
)
...
func TestYouHaveBeenRemindedToAddABreakingChangelogEntryOnKurtCoreAPIBreak(t *testing.T) {
	actualEngineSemver, err := semver.Parse(engine_server_launcher.DefaultVersion)
	require.NoError(t, err, "An unexpected error occurred parsing engine server version string '%v'", engine_server_launcher.DefaultVersion)
