...
func (h *Handler) ServeHTTP(responseWriter http.ResponseWriter, request *http.Request) {
	...
	if timeoutErr != nil {
		logrus.Infof("[LEO-DEBUG] timeout error when setting timeout")
		ctx = request.Context()
	}
	if cancel != nil {
		defer func() {
			logrus.Infof("[LEO-DEBUG] calling cancel in handler")
			cancel()
		}()
	}
	...
	if !ok {
		logrus.Infof("[LEO-DEBUG] failed to create stream")
		// Failed to create stream, usually because client used an unknown
		// compression algorithm. Nothing further to do.
		return
	}
	if timeoutErr != nil {
		logrus.Infof("[LEO-DEBUG] closing connection with timeout err != nil")
		_ = connCloser.Close(timeoutErr)
		return
	}
	logrus.Infof("[LEO-DEBUG] regular closing connection")
