...
1. The `--privileged` flag allows Docker-only `privileged`, `bind_mounts`, `host_pid_namespace`, and `host_cgroup_namespace` fields to be preserved or enabled during the update. This is an allow flag: it does not make the service privileged by itself.
...
Updating a service that already has `privileged: true`, `bind_mounts`, `host_pid_namespace: true`, or `host_cgroup_namespace: true` requires `--privileged` or `allow-privileged-mode: true` in the current cluster's `kurtosis-config.yml`, otherwise the update is rejected while the updated service config is interpreted. `service update` currently preserves and can enable these fields, but it cannot clear `privileged: true`, remove existing bind mounts, or unset `host_pid_namespace: true` / `host_cgroup_namespace: true`. To remove privileged access today, recreate the service without those fields.
:::
