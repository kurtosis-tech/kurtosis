...
2. The `--parallel` flag enables Kurtosis to run instructions in parallel, rather than one after another. Kurtosis will analyze the dependencies between instructions and execute each instruction as soon as its dependencies are complete, resulting in faster overall execution. **Note:** This feature is experimental and may encounter issues in certain scenarios.
...
1. The `--privileged` flag allows Docker-only [`ServiceConfig.privileged`](../api-reference/starlark-reference/service-config.md), [`ServiceConfig.bind_mounts`](../api-reference/starlark-reference/service-config.md), `ServiceConfig.host_pid_namespace`, and `ServiceConfig.host_cgroup_namespace` fields during this run. This is an allow flag: it does not make every service privileged by itself. A package must still explicitly set `privileged=True`, `bind_mounts={...}`, `host_pid_namespace=True`, or `host_cgroup_namespace=True` in its `ServiceConfig`. You can also allow these fields for all CLI runs against a configured cluster by setting `allow-privileged-mode: true` in `kurtosis-config.yml`.

1. The `--resource-check` flag can be used to control whether Kurtosis checks available CPU and memory before execution. Defaults to `true`. Disable with `--resource-check=false` to skip the check when you know resources are sufficient.
