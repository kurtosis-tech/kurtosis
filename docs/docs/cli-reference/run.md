...
1. The `--image-download` flag can be used to determine what is the behavior to fetch images tagged as latest during the package deployment. Use `missing` to fetch the latest image only if no other version with the latest tag is available in local cache and use `always` to always fetch the latest image from remote repository (default behavior is `missing`).
