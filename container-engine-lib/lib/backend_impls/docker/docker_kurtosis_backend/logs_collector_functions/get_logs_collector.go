...
func GetLogsCollectorForEnclave(
	...
	*/
	...
	bridgeNetwork, err := dockerManager.GetNetworksByName(ctx, "bridge")
	if err != nil {
		return nil, stacktrace.Propagate(err, "An error occurred while retrieving the network id for the enclave '%v'", enclaveUuid)
	}

	maybeLogsCollectorObject, _, err := getLogsCollectorObjectAndContainerId(ctx, enclaveUuid, bridgeNetwork[0], dockerManager)
	if err != nil {
