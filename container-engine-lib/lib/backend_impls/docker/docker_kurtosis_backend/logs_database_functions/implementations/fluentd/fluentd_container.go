...
const (
	maxWaitForFluentdServiceAvailabilityRetries         = 10
	timeBetweenWaitForFluentdServiceAvailabilityRetries = 1 * time.Second
)
...
func (lokiContainer *fluentdContainer) CreateAndStart(
	ctx context.Context,
	httpPortId string,
	...
	fluentdContainerConfigProviderObj := createFluentdContainerConfigProviderForKurtosis(httpPortNumber)

	privateHttpPortSpec, err := fluentdContainerConfigProviderObj.GetPrivateHttpPortSpec()
	if err != nil {
	...
	createAndStartArgs, err := fluentdContainerConfigProviderObj.GetContainerArgs(containerName, containerLabelStrs, volumeName, targetNetworkId)
	if err != nil {
		return "", nil, nil,
	...
	// TODO: replace that with the official wait in ServiceConfig maybe?
	if err = shared_helpers.WaitForPortAvailabilityUsingNetstat(
		ctx,
		dockerManager,
		containerId,
		privateHttpPortSpec,
		maxWaitForFluentdServiceAvailabilityRetries,
		timeBetweenWaitForFluentdServiceAvailabilityRetries,
	); err != nil {
