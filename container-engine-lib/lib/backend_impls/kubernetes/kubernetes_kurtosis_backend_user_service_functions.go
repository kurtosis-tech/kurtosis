...
func (backend *KubernetesKurtosisBackend) getUserServiceVolumeInfoFromFilesArtifactMountpoints(
	...
	filesArtifactExpansionPvcSearchLabels := map[string]string{
		label_key_consts.EnclaveIDKubernetesLabelKey.GetString(): string(enclaveId),
		label_key_consts.KurtosisVolumeTypeKubernetesLabelKey.GetString(): label_value_consts.FilesArtifactsExpansionVolumeTypeKubernetesLabelValue.GetString(),
...
// Takes in a list of mountpoints on the expander container that the expander container wants populated,
