...
func (resource serviceKubernetesResource) getLabels() map[string]string {
	return resource.underlying.Labels
}
func (resource serviceKubernetesResource) getUnderlying() interface{} {
...
func CollectMatchingServices(
	...
	for _, object := range allObjects.Items {
		allKubernetesResources = append(
			...
			serviceKubernetesResource{underlying: object},
		)
	}
	...
	for labelValue, uncastedResource := range filteredKubernetesResources {
		castedResource, ok := uncastedResource.getUnderlying().(apiv1.Service)
		if !ok {
