...
type serviceAccountKubernetesResource struct {
	underlying apiv1.ServiceAccount
}
func (resource serviceAccountKubernetesResource) getName() string {
	return resource.underlying.Name
}
func (resource serviceAccountKubernetesResource) getLabels() map[string]string {
	return resource.underlying.Labels
}
func (resource serviceAccountKubernetesResource) getUnderlying() interface{} {
...
func CollectMatchingServiceAccounts(
	...
	allObjects, err := kubernetesManager.GetServiceAccountsByLabels(ctx, namespace, searchLabels)
	if err != nil {
	...
	for _, object := range allObjects.Items {
		allKubernetesResources = append(
			...
			serviceAccountKubernetesResource{underlying: object},
		)
	}
	...
	result := map[string][]*apiv1.ServiceAccount{}
	for labelValue, matchingResources := range filteredKubernetesResources {
		castedObjects := []*apiv1.ServiceAccount{}
		for _, resource := range matchingResources {
			casted, ok := resource.getUnderlying().(apiv1.ServiceAccount)
			if !ok {
