...
type clusterRoleBindingKubernetesResource struct {
	underlying rbacv1.ClusterRoleBinding
}
func (resource clusterRoleBindingKubernetesResource) getName() string {
	return resource.underlying.Name
}
func (resource clusterRoleBindingKubernetesResource) getLabels() map[string]string {
	return resource.underlying.Labels
}
func (resource clusterRoleBindingKubernetesResource) getUnderlying() interface{} {
...
func CollectMatchingClusterRoleBindings(
	...
	for _, object := range allObjects.Items {
		allKubernetesResources = append(
			...
			clusterRoleBindingKubernetesResource{underlying: object},
		)
	}
	...
	for labelValue, uncastedResource := range filteredKubernetesResources {
		 castedResource, ok := uncastedResource.getUnderlying().(rbacv1.ClusterRoleBinding)
		 if !ok {
