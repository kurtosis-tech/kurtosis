...
type persistentVolumeClaimKubernetesResource struct {
	underlying apiv1.PersistentVolumeClaim
}
func (resource persistentVolumeClaimKubernetesResource) getName() string {
	return resource.underlying.Name
}
func (resource persistentVolumeClaimKubernetesResource) getLabels() map[string]string {
	return resource.underlying.Labels
}
func (resource persistentVolumeClaimKubernetesResource) getUnderlying() interface{} {
	return resource.underlying
}

// TODO Remove all this when we have Go 1.18 generics
// NOTE: This function is intended to be copy-pasted to create new ones
func CollectMatchingPersistentVolumeClaims(
	ctx context.Context,
	...
	map[string][]*apiv1.PersistentVolumeClaim,
	error,
) {
	allObjects, err := kubernetesManager.GetPersistentVolumeClaimsByLabels(ctx, namespace, searchLabels)
	if err != nil {
	...
	for _, object := range allObjects.Items {
		allKubernetesResources = append(
			...
			persistentVolumeClaimKubernetesResource{underlying: object},
		)
	}
	...
	result := map[string][]*apiv1.PersistentVolumeClaim{}
	for labelValue, matchingResources := range filteredKubernetesResources {
		castedObjects := []*apiv1.PersistentVolumeClaim{}
		for _, resource := range matchingResources {
			casted, ok := resource.getUnderlying().(apiv1.PersistentVolumeClaim)
			if !ok {
