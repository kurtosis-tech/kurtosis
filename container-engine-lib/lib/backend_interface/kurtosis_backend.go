...
type KurtosisBackend interface {
	...
	*/
	...
	RemoveRegisteredUserServiceProcesses(
		...
		map[service.ServiceUUID]bool, // the updated "set" of user UUIDs that were successfully updated
		map[service.ServiceUUID]error, // "set" of user service UUIDs that errored when attempting to update, with the error
		error, // represents an error with the function itself, rather than the user services
	)
