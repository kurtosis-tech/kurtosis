# TBD
### Breaking Changes
* Updated `CreateEngine` method, removed the `logsCollectorHttpPortNumber` parameter
  * Users will need to update all the `CreateEngine` calls removing this new parameter
  
### Features
* Created the `LogsDatabase` object
* Created the `LogsCollector` object

### Changes
* Untied the logs components containers and volumes creation and removal from the engine's crud
...
### Fixes
* Fixed a bug where `KurtosisBackend.StartUserServices` would destroy all services regardless of success or failure.
