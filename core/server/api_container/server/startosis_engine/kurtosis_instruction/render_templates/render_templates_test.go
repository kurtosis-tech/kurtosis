...
func TestRenderTemplate_TestStringRepresentation(t *testing.T) {
	...
	renderInstruction.starlarkKwargs[nonOptionalArtifactNameArgName] = starlark.String(testArtifactName)
	...
	expectedStr := `render_templates(config=` + expectedConfig + `, name="` + testArtifactName + `")`
