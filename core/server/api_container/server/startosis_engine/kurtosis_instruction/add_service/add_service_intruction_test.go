...
func TestAddServiceInstruction_SerializeAndParseAgain_UnamedStruct(t *testing.T) {
	serviceConfigBuilder := services.NewServiceConfigBuilder(
		testContainerImageName,
	).WithPrivatePorts(map[string]*kurtosis_core_rpc_api_bindings.Port{
		"grpc": binding_constructors.NewPort(1234, kurtosis_core_rpc_api_bindings.Port_TCP, "http"),
...
func newTestStarlarkServiceConfigWithUnamedStruct(t *testing.T) *starlarkstruct.Struct {
	...
	serviceConfigDict["private_ip_address_placeholder"] = starlark.String(testIpAddressPlaceholder)
	serviceConfigDict["subnetwork"] = starlark.String(testSubnetwork)
