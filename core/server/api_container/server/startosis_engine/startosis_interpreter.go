...
func (interpreter *StartosisInterpreter) InterpretAndOptimizePlan(
	...
	for {
		...
		matchingInstructionIdx, err := findFirstEqualInstructionPastIndex(currentEnclavePlanSequence, naiveInstructionsPlanSequence, firstPossibleIndexForMatchingInstruction, interpreter.enclavePlanInstructionRepository)
		if err != nil {
			return startosis_constants.NoOutputObject, nil, startosis_errors.WrapWithInterpretationError(err, "An error occurred finding the first equal instruction past index").ToAPIType()
		}
...
func (interpreter *StartosisInterpreter) Interpret(
	...
	if mainFuncParamsNum >= minimumParamsRequiredForPlan {
		...
		if firstParamName == planParamName {
			...
			planModule := plan_module.PlanModule(newInstructionsPlan, enclaveComponents, instructionsPlanMask, kurtosisPlanInstructions, interpreter.enclavePlanInstructionRepository)
...
func findFirstEqualInstructionPastIndex(
