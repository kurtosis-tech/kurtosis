...
func KurtosisHelpers(packageId string, moduleAbsoluteLocator string, recursiveInterpret func(packageId string, moduleId string, scriptContent string) (starlark.StringDict, *startosis_errors.InterpretationError), packageContentProvider startosis_packages.PackageContentProvider, packageGlobalCache map[string]*startosis_packages.ModuleCacheEntry) []*starlark.Builtin {
	return []*starlark.Builtin{
		...
		starlark.NewBuiltin(read_file.ReadFileBuiltinName, read_file.NewReadFileHelper(packageId, moduleAbsoluteLocator, packageContentProvider).CreateBuiltin()),
	}
}
