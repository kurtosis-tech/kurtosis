...
type serviceConfigHostCgroupNamespaceTestCase struct {
	*testing.T
	serviceNetwork         *service_network.MockServiceNetwork
	packageContentProvider *startosis_packages.MockPackageContentProvider
}

func (suite *KurtosisTypeConstructorTestSuite) TestServiceConfigWithHostCgroupNamespace() {
	suite.run(&serviceConfigHostCgroupNamespaceTestCase{
		T:                      suite.T(),
...
func (t *serviceConfigHostCgroupNamespaceTestCase) GetStarlarkCode() string {
	return fmt.Sprintf("%s(%s=%q, %s=True)",
		service_config.ServiceConfigTypeName,
		service_config.ImageAttr, testContainerImageName,
		service_config.HostCgroupNamespaceAttr)
}

func (t *serviceConfigHostCgroupNamespaceTestCase) Assert(typeValue builtin_argument.KurtosisValueType) {
	...
	expectedServiceConfig.SetHostCgroupNamespace(true)
	require.Equal(t, expectedServiceConfig, serviceConfigResult)
	require.True(t, serviceConfigResult.GetHostCgroupNamespace())
}
