...
type serviceConfigPrivilegedTestCase struct {
	*testing.T
	serviceNetwork         *service_network.MockServiceNetwork
	packageContentProvider *startosis_packages.MockPackageContentProvider
}

func (suite *KurtosisTypeConstructorTestSuite) TestServiceConfigWithPrivileged() {
	suite.run(&serviceConfigPrivilegedTestCase{
		T:                      suite.T(),
...
func (t *serviceConfigPrivilegedTestCase) GetStarlarkCode() string {
	return fmt.Sprintf("%s(%s=%q, %s=True)",
		service_config.ServiceConfigTypeName,
		service_config.ImageAttr, testContainerImageName,
		service_config.PrivilegedAttr)
}

func (t *serviceConfigPrivilegedTestCase) Assert(typeValue builtin_argument.KurtosisValueType) {
	...
	expectedServiceConfig.SetPrivileged(true)
	require.Equal(t, expectedServiceConfig, serviceConfigResult)
	require.True(t, serviceConfigResult.GetPrivileged())
}
