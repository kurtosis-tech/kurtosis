...
func testKurtosisPlanInstruction(t *testing.T, builtin KurtosisPlanInstructionBaseTest) {
	...
	instructionPlan := instructions_plan.NewInstructionsPlan()
	...
	emptyInstructionsMask := resolver.NewInstructionsPlanMask(0)
	instructionWrapper := kurtosis_plan_instruction.NewKurtosisPlanInstructionWrapper(instructionFromBuiltin, instructionPlan, emptyInstructionsMask)
	...
	require.Equal(t, 1, instructionPlan.Size())
	instructionQueue, err := instructionPlan.GeneratePlan()
	require.Nil(t, err)
	instructionToExecute := instructionQueue[0].GetInstruction()
