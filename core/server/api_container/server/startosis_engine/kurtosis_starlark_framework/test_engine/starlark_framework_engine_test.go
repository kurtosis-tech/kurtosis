...
func testKurtosisPlanInstruction(t *testing.T, builtin KurtosisPlanInstructionBaseTest) {
	...
	enclaveComponentsStore := enclave_structure.NewEnclaveComponents()
	emptyInstructionsPlanMask := resolver.NewInstructionsPlanMask(0)
	instructionWrapper := kurtosis_plan_instruction.NewKurtosisPlanInstructionWrapper(instructionFromBuiltin, emptyInstructionsPlanMask, enclaveComponentsStore, instructionsPlan)
