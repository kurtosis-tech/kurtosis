...
type serviceConfigHostPIDNamespaceTestCase struct {
	*testing.T
	serviceNetwork         *service_network.MockServiceNetwork
	packageContentProvider *startosis_packages.MockPackageContentProvider
}

func (suite *KurtosisTypeConstructorTestSuite) TestServiceConfigWithHostPIDNamespace() {
	suite.run(&serviceConfigHostPIDNamespaceTestCase{
		T:                      suite.T(),
...
func (t *serviceConfigHostPIDNamespaceTestCase) GetStarlarkCode() string {
	return fmt.Sprintf("%s(%s=%q, %s=True)",
		service_config.ServiceConfigTypeName,
		service_config.ImageAttr, testContainerImageName,
		service_config.HostPIDNamespaceAttr)
}

func (t *serviceConfigHostPIDNamespaceTestCase) Assert(typeValue builtin_argument.KurtosisValueType) {
	...
	expectedServiceConfig.SetHostPIDNamespace(true)
	require.Equal(t, expectedServiceConfig, serviceConfigResult)
	require.True(t, serviceConfigResult.GetHostPIDNamespace())
}
