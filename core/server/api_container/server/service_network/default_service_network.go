...
func (network *DefaultServiceNetwork) destroyService(ctx context.Context, serviceName service.ServiceName, serviceUuid service.ServiceUUID) error {
	...
	if err != nil {
		return stacktrace.Propagate(err, "Attempted to destroy the service with UUID '%v' but had no success. You must manually destroy the service as well as its sidecar.", serviceUuid)
	}
	if _, found := successfullyDestroyedUuids[serviceUuid]; found {
		...
		if failedToDestroyErr, found := failedToDestroyUuids[serviceUuid]; found {
			return stacktrace.Propagate(failedToDestroyErr, "Attempted to destroy the service with UUID '%v' but had no success. You must manually destroy the service as well as its sidecar", serviceUuid)
		} else {
