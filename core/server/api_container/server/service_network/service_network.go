...
func (network *ServiceNetwork) RemoveService(
	...
	if err != nil {
		return nil, stacktrace.Propagate(err, "An error occurred during the call to stop service '%v'", serviceGuid)
	}
	if err, found := erroredGuids[serviceGuid]; found {
		return nil, stacktrace.Propagate(err, "An error occurred stopping service '%v'", serviceGuid)
	}
	...
	if network.isPartitioningEnabled && foundSidecar {
		...
		if err := network.networkingSidecarManager.Remove(ctx, sidecar); err != nil {
			return nil, stacktrace.Propagate(err, "An error occurred destroying the sidecar for service with ID '%v'", serviceId)
		}
