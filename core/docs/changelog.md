...
# 1.59.2
### Changes
* Upgraded container-engine-lib to `0.38.1`
...
* Made `ServiceNetwork.StartServices` clean up after itself completely
  * Destroys all the services that fail anywhere in the process.
  * Removes failing services from the partition topology.
  * Removes the networking sidecars of any services that fail anywhere in the process.
  * Undoes the partitioning, and removes the failing services from the `qdisc` rules of other services.
...
* `EnclaveContext.UploadFiles` adds the contents of the directory to the root of the archive instead of nesting contents inside the directory inside the archive.
  ...
  * Users of the sdk should note that they can't upload empty directories
