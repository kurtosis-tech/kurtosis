...
func TestStartosis(t *testing.T) {
	...
	outputStream, _, err := enclaveCtx.ExecuteStarlarkScript(ctx, startosisScript, defaultDryRun)
...
`
	...
	outputStream, _, err = enclaveCtx.ExecuteStarlarkScript(ctx, removeScript, defaultDryRun)
