...
def run(plan, args):
	...
	rendered_artifact = plan.render_templates(template_data_by_path, artifact_id = "rendered-file")
	plan.print("Rendered file to " + rendered_artifact)
...
func TestStartosis_IsBackwardsCompatible(t *testing.T) {
	ctx := context.Background()
	...
	runResult, err := enclaveCtx.RunStarlarkScriptBlocking(ctx, starlarkScriptWithOldSyntax, greetingsArg, defaultDryRun)
	require.NoError(t, err, "Unexpected error executing startosis script")
