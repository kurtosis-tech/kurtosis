...
func TestAddServiceWithEmptyPortsAndWithoutPortsWithStruct(t *testing.T) {
	ctx := context.Background()
	...
	// ------------------------------------- TEST RUN ----------------------------------------------
	result, err := enclaveCtx.RunStarlarkScriptBlocking(ctx, subnetworkInStarlarkScriptWithStruct, emptyArgs, executeNoDryRun)
	require.Nil(t, err, "Unexpected error happened executing Starlark script")
