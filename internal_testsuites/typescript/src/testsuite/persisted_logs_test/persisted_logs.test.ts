...
async function TestSearchLogs() {
    ...
    try {
        ...
        for (let [serviceName, serviceCtx] of serviceList) {
            serviceUuid = serviceCtx.getServiceUUID();
            userServiceUuids.add(serviceUuid);

            // NOW REMOVE SERVICE
            const removeServiceResult: Result<Error> = await removeServiceViaStarlark(enclaveContext, serviceName);
            if (removeServiceResult.isErr()){
                throw new Error(`An error occurred removing the services for the test. Error:\n${removeServiceResult.error}`);
            }
        }
